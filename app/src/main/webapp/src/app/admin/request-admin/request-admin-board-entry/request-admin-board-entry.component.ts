...
import {BookingRequestChangeResult, BookingRequestInfo, defaultBookingRequestInfo} from "../model/request-admin-api";
...
export class RequestAdminBoardEntryComponent {
  ...
  private handleConfirmed(result: BookingRequestChangeResult) {
    this.changing = false
  ...
  private handleDenied(result: BookingRequestChangeResult) {
    this.changing = false
    let message = this.translationService.instant(result.msg)
    this.snackBar.open(message, 'OK', this.snackbarConfig)
      .afterDismissed()
