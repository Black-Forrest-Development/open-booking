...
class SettingsController(private val service: SettingsService) : SettingsAPI {
    ...
    @Get("/{id}")
    override fun get(auth: Authentication, @PathVariable id: String) = auth.checkPermission(PERMISSION_READ) {
        service.get(id)
    }
    ...
    @Put("/{id}")
    override fun update(auth: Authentication, @PathVariable id: String, @Body request: SettingChangeRequest) = auth.checkPermission(PERMISSION_WRITE) {
        service.update(id, request)
    }

    @Delete("/{id}")
    override fun delete(auth: Authentication, @PathVariable id: String) = auth.checkPermission(PERMISSION_WRITE) {
        service.delete(id)
    }

    @Patch("/{id}/value")
    override fun setValue(auth: Authentication, @PathVariable id: String, @Body value: PatchRequest<Any>)=
